...
static int digit()
{
	...
	if (!is_digit(token)) {
		ERROR("Expected digit. Received %c\n", token);
		exit(EXIT_FAILURE);
	}
	reg = next_register();
...
static int variable()
{
	...
	if (!is_identifier(token)) {
		ERROR("Expected identifier. Received %c\n", token);
		exit(EXIT_FAILURE);
	}
	reg = next_register();
...
static void assign()
{
	...
	//Temp should always be a variable
	temp = token;
	if (temp < 'a' || temp > 'z') {
		ERROR("Expected variable. Received %c\n", temp);
		exit(EXIT_FAILURE);
	}
	next_token();
	...
	//Check '='
	if (token != '=') {
		ERROR("Expected '-'. Received %c\n", token);
		exit(EXIT_FAILURE);
	}
	next_token();
	right_reg = expr();

	//left_reg is the variable's register
	//right_reg contains the value we want to put in left_reg
	//right_reg memory location is r0 (1024) + right_reg*4 (offset in bytes)
...
static void print()
{
	...
	//Next should always be a variable
	temp = token;
	if (temp < 'a' || temp > 'z') {
		ERROR("Expected variable. Received %c\n", temp);
		exit(EXIT_FAILURE);
	...
	//reg memory location is r0 (1024) + right_reg*4 (offset in bytes)
	CodeGen(OUTPUTAI, 0, (temp-'a')*4, EMPTY_FIELD);
}
